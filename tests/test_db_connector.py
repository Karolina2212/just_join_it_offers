...
class TestDBConnector(unittest.TestCase):
    ...
    def test_db_insert_one(self):
        ...
        sql_insert_check = sql.SQL("SELECT jjit_id, title, company_name, marker_icon, workplace_type, experience_level, import_date, country_code FROM {table} WHERE id = %s").format(
    ...
    def test_db_update_row(self):
        self.db_connector.update_row('offers_info',
                                     {'jjit_id': 'test_1'},
                                     {'title': 'updated_title', 'company_name': 'updated_company_name'})
